...
class MFDataset(data.Dataset):
    def __init__(self, root_path, transform, type="test"):
        # 该方法主要用于生成三个列表，分别装载指定文件夹下所有图片的绝对路径/相对路径

        self.transforms = transform
        self.type = type

        img_list1 = os.listdir(root_path + "/part1")
        img_list1.sort()

        img_list2 = os.listdir(root_path + "/part2")
        img_list2.sort()

        self.img_pt1_path = [root_path + '/part1/' + image for image in img_list1]
        self.img_pt2_path = [root_path + '/part2/' + image for image in img_list2]

        if self.type== "train":
            img_list3 = os.listdir(root_path + "/clear")
            img_list3.sort()
            self.img_pt3_path = [root_path + '/clear/' + image for image in img_list2]
